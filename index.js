...
export function hotmods(opts, listener) {
  ...
  updates(async ({ diff }) => {
    ...
    for (let update of diff) {
      ...
      mods[key].original =
        mods[key].original || (await import(origin + key + '+app+esm'))
      mods[key].previous = mods[key].next || mods[key].original
      try {
        mods[key].next = await import(
          origin + key + `?refresh=${Date.now()}+app+esm`
        )
      } catch (err) {
        console.error('Error live-reloading', key, err)
      }
      mods[key].reloads = Array.from(
        new Set(
          Object.keys(mods[key].original).concat(
            Object.keys(mods[key].previous)
          )
        )
      ).map((name) => ({
        type,
    ...
    const reloads = Array.from(
      new Set(
        Object.values(mods).reduce((reloads, hotmod) => {
